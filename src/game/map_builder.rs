...
use rand::Rng;
use crate::game::tiles::{FloorTiles, TileKind};
use crate::map::{PoirogueMap, PoirogueMapBuilder};
use std::collections::{HashMap};
use petgraph::{Graph, Undirected};
use petgraph::graph::NodeIndex;
use petgraph::algo::min_spanning_tree;
use petgraph::data::FromElements;
use crate::game::mobs::Mob;
use crate::game::player::Player;

pub struct MazeBuilderConfig {
    pub room_creation_attempts: i32,
    pub min_room_size: (i32, i32),
    pub max_room_size: (i32, i32),
}

pub struct SimpleDungeonBuilder {
    pub rects: Vec<Rect>,
...
impl SimpleDungeonBuilder {

    pub fn new() -> SimpleDungeonBuilder {
    ...
    fn fill_rooms(&mut self, config: MazeBuilderConfig, map: &mut PoirogueMap) {
        ...
        for _attempt in 0..config.room_creation_attempts {
            ...
            if okay {
                ...
                for p in room.point_set() {
                    map.set(p.x, p.y, TileKind::RectRoom(index));
                }
    ...
    fn link_rects_into_rooms(&mut self, map: &mut PoirogueMap) {
        ...
        #[inline(always)]
        fn flood_fill(builder: &mut SimpleDungeonBuilder, map: &mut PoirogueMap, x: i32, y: i32, fill: u8) {
            ...
            match &map.tiles[index] {
                TileKind::RectRoom(_) => {
                    ...
                    map.tiles[index] = TileKind::Floor(fill, internal_tile);
    ...
    fn draw_corridor(&mut self, map: &mut PoirogueMap, p: Point, q: Point) {
        ...
        for c in line {
            ...
            match map.tiles[index] {
                TileKind::Obscured(_)  => { map.set(c.x, c.y, TileKind::Corridor); }
                _ => {}
            }
        }
    }

    pub fn get_random_room_center(&self) -> &Point {
        let mut rng = rand::thread_rng();
    ...
    pub fn place_objects(&mut self, map: &mut PoirogueMap) {
        let mut rng = rand::thread_rng();
...
impl PoirogueMapBuilder for SimpleDungeonBuilder {
    fn generate(&mut self, map: &mut PoirogueMap) {
        self.fill_rooms(MazeBuilderConfig{ room_creation_attempts: 10, min_room_size: (10, 8), max_room_size: (15, 12) }, map);
        self.fill_rooms(MazeBuilderConfig{ room_creation_attempts: 200, min_room_size: (5, 5), max_room_size: (8, 7) }, map);
        self.fill_rooms(MazeBuilderConfig{ room_creation_attempts: 300, min_room_size: (4, 4), max_room_size: (6, 5) }, map);
        self.link_rects_into_rooms(map);
        self.link_neighbors(map);

        map.entities.push(Box::new(Player::new(*self.get_random_room_center())));
